...
class Main {
    ...
    // 경우의 수를 구하는 메서드
    // 주어진 coin 유형을 가진 coins 배열과 정수 합계 sum을 매개변수로 받아서 계산
    public static int countWaysToMakeSum(int sum, int[] coins) {
        // 1. dp 배열 초기화
        // dp 배열: 주어진 합계(sum)를 만드는 coin 조합의 개수를 저장하기 위한 배열 dp
        int[] dp = new int[sum + 1];

        // 2. dp[0] 초기화
        // 2.1. 현재 coin 유형만 사용한 경우(i - coin = 0) = 1가지
        dp[0] = 1;

        // dp[sum] = sum을 만드는데 사용된 coin 조합 경우의 수
    ...
    // 매개변수: 합계, 코인 배열, 현재 조합 리스트, 최종 결과 리스트, index 시작점
